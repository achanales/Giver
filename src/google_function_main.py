...
def return_ranked_charities(request):
    ...
    ##### Download, Unzip and Load Spacy Language Model  ####

    download_blob('charity_recommender', 'en_core_web_sm.zip', '/tmp/en_core_web_sm.zip')

    print('Unzipping en_core')
    with zipfile.ZipFile('/tmp/en_core_web_sm.zip', 'r') as zip_ref:
        zip_ref.extractall('/tmp/')
    print('Unzipped en_core')

    print('Loading nlp')
    nlp = spacy.load('/tmp/en_core_web_sm/en_core_web_sm-2.1.0')
    print('Loaded nlp')
    ...
    print('Preprocessing headline')
    # Preprocess headline
    headline_pre = preprocess_spacy(headline, nlp)
    print('Preprocessed headline')

    download_blob('charity_recommender', 'charity_model_min_0_max_0.5_notfidf.pickle',
                  '/tmp/charity_model_min_0_max_0.5_notfidf.pickle')
    ...
    os.remove('/tmp/en_core_web_sm.zip')
    shutil.rmtree('/tmp/en_core_web_sm')

    headline_keywords = headline_pre + ' ' + ', '.join(keywords)
    print(headline_keywords)

    # embed in document vector
    headline_keywords_emb = embed_text(headline_pre, charity_docs_dict, charity_emb_vecs)
    ...
    topN_charities['keywords'] = keywords
